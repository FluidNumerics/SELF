...
MODULE SELF_Memory
  ...
  !! Data type for storing one-dimensional int32 arrays on the host and the device
    ...
    integer(int32), pointer :: deviceData(:)
...
CONTAINS
  ...
  SUBROUTINE Alloc_hfReal_r1(this,loBound,upBound)
    ...
    this % hostData = 0.0_prec

    IF (GPUAvailable()) THEN
      CALL hipCheck(hipMalloc(this % deviceData,mold=this % hostdata))
    END IF
  ...
  SUBROUTINE UpdateDevice_hfReal_r1(this)
    ...
    IF (GPUAvailable()) THEN
      CALL hipCheck(hipMemcpy(c_loc(this % devicedata), &
                              c_loc(this % hostData), &
                              sizeof(this % hostdata), &
