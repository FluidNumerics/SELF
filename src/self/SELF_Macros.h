...
#define TE_3D_INDEX(row,col,i,j,k,iVar,iel,N,nVar) row-1 + 2*(col-1 + 2*(i + (N+1)*(j + (N+1)*(k + (N+1)*(iVar + nVar*iel)))))
...
#define TEB_3D_INDEX(row,col,i,j,iVar,iSide,iel,N,nVar) row-1 + 2*(col-1 + 2*(i + (N+1)*(j + (N+1)*(iVar + nVar*(iSide-1 + 6*iel)))))
