...
!
 SUBROUTINE InitialCondition( myDGSEM, myRank )
   ...
   REAL(prec) :: x, y, z, Lx, Ly, r
         ...
         DO k = 0, myDGSEM % N
            DO j = 0, myDGSEM % N
               DO i = 0, myDGSEM % N
                  ...
                  myDGSEM % state % solution(i,j,k,6,iEl) = myDGSEM % static % solution(i,j,k,6,iEl)                 
               ENDDO
            ENDDO
         ENDDO
      ENDDO
      ...
      ! For this demo we calculate the tendency (pressure gradient force) and store the dpdx in "u" and 
      ! dpdy in "v"

      CALL myDGSEM % GlobalTimeDerivative( 0.0_prec )
         ...
         DO k = 0, myDGSEM % N
            DO j = 0, myDGSEM % N
               DO i = 0, myDGSEM % N
                 ...
                 myDGSEM % state % solution(i,j,k,1,iEl) = myDGSEM % state % tendency(i,j,k,1,iEl)
                 myDGSEM % state % solution(i,j,k,2,iEl) = myDGSEM % state % tendency(i,j,k,2,iEl)
