...
USE SELF_Model2D
  ...
  INTEGER :: iEl, i, j 
    ...
    initialCondition = (/"s = sin(2.0*pi*x/100000.0)*sin(2.0*pi*y/100000.0)"/)
    CALL semModel % SetSolution( initialCondition )

    ! Set the exact solution
    CALL exact % solutionGradient % SetEquation( 1, 1, &
                     "s = (2.0*pi/100000.0)*cos(2.0*pi*x/100000.0)*sin(2.0*pi*y/100000.0)" )

    CALL exact % solutionGradient % SetEquation( 2, 1, &
                     "s = (2.0*pi/100000.0)*sin(2.0*pi*x/100000.0)*cos(2.0*pi*y/100000.0)" )

    CALL exact % solutionGradient % SetInteriorFromEquation( geometry, 0.0_prec )


    ! Estimate gradient using Bassi-Rebay method
    CALL semModel % solution % SideExchange(mesh, decomp, semModel % gpuAccel)
    ...
    maxXerr = 0.0_prec
    maxYerr = 0.0_prec
    DO iEl = 1, semModel % solution % nElem
      DO j = 0, semModel % solution % interp % N
        DO i = 0, semModel % solution % interp % N

          absDiff = ABS(semModel % solutionGradient % interior % hostData(1,i,j,1,iEl) - &
                        exact % solutionGradient % interior % hostData(1,i,j,1,iEl) )
          maxXerr = MAX( maxXerr,  absDiff )

          absDiff = ABS(semModel % solutionGradient % interior % hostData(2,i,j,1,iEl) - &
                        exact % solutionGradient % interior % hostData(2,i,j,1,iEl) )
          maxYerr = MAX( maxYerr,  absDiff )

        ENDDO
      ENDDO
    ENDDO

    PRINT*, maxXerr, maxYerr
