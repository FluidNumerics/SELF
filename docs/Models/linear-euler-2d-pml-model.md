...
* `flux2d`, `riemannflux2d` — use the parent linear-Euler acoustic flux form for variables 1–4 and return zero for variables 5–7 (auxiliaries carry no flux). Note the PML variant uses the **scalar** background density `rho0`: unlike the parent model (which carries `rho0` as a per-node solution variable in slot 5 and a per-node relaxation rate `sigma` in slot 6), the PML model repurposes slots 5 and 6 for the auxiliaries `phi_u` and `phi_v`. The PML provides its own absorption mechanism through `sigma_x`/`sigma_y`, so the parent's sponge-layer relaxation variable is not carried.
