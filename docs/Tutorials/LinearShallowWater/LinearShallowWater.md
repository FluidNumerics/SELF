...
You can find the example file for this demo in the `examples/shallow_water_2d.f90` file. This file uses the `shallow_water_2d` module from `src/SELF_shallow_water_2d_t.f90`.

No normal flow conditions are built into the `shallow_water_2d` module when we assign `hbc2d_NoNormalFlow => hbc2d_NoNormalFlow_shallow_water_2d_t`:
...
```shell
${SELF_ROOT}/examples/shallow_water_2d
```
